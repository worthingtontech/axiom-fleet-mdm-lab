...
This is the layer where AXIOM stops *observing* and starts *acting*. Everything below it — the agent ([fleet-core](./03-fleet-core.md)), the policies ([policy-as-code](./07-policy-as-code.md)), the logs and metrics ([telemetry](./08-telemetry-and-observability.md)) — produces signals; this layer closes the loop by turning a signal into a change on a host, automatically where it's safe and via a human-reviewed ticket where it isn't. The whole loop is built from **two things Fleet Free gives you for free** (outbound webhooks and the script-execution API) glued together by a tiny FastAPI receiver we call **SOAR-lite**. Because both primitives are Free, the entire Phase 8 auto-remediation story is *designed* to run at $0 — the honest caveats are that the *ergonomic* MDM lock/wipe actions and CVE risk-*scores* are Premium, and that Fleet's default webhook cadence (once per day) has to be turned down before any of this feels like "incident response."

> **Build status — read this first.** *SOAR-lite is an **intended design / blueprint**, not a running component in this lab.* There is **no `automation/` service**, and Fleet's failing-policies webhook that would feed it is **authored but DISABLED** (`gitops/default.yml`: `enable_failing_policies_webhook: false`). What is **actually shipped** for remediation is (a) the free **Fleet run-script API** for manual/scripted fixes (e.g. `gitops/lib/linux/scripts/install-auditd.sh`), and (b) a **Claude-in-the-loop** flow in `gitops/remediate/` (`claude-remediate.ps1` + `.github/workflows/claude-remediate.yml`) that drafts a fix as a **human-reviewed pull request**. Read the SOAR-lite sections below as the design AXIOM *would* build, not a description of a deployed service.
...
flowchart LR
  ...
  subgraph core["axiom-core Docker network"]
    ...
    fleet["Fleet :1337"]
    soar["SOAR-lite :8080<br/>(design — not deployed)"]
    db[("MySQL")]
  end
  ...
  - **Failing-policies webhook** (`webhook_settings.failing_policies_webhook`): on each check cycle, Fleet finds policies that are *newly failing* and POSTs a payload naming the policy and the affected hosts. A representative (classic, grouped-by-policy) body:
    ```json
    {
      ...
      "hosts": [ { "id": 7, "hostname": "enclave-01",
                   "url": "https://fleet.axiom.lab/hosts/7" } ]
    }
    ```
...
> **Status: design, not deployed.** The SOAR-lite receiver described in this section is a **blueprint**, not a running service — AXIOM does **not** run an `automation/` FastAPI container, and the failing-policies webhook that would feed it is **disabled** (`enable_failing_policies_webhook: false` in `gitops/default.yml`). The remediation that **is** built and shipped is the free **Fleet run-script API** (manual/scripted, e.g. `gitops/lib/linux/scripts/install-auditd.sh`) plus a **Claude-in-the-loop** flow (`gitops/remediate/claude-remediate.ps1` + `.github/workflows/claude-remediate.yml`) that turns an alert into a **human-reviewed PR**. Treat the routes, dispatch table, and diagrams below as the intended design.

- **In one line** — **SOAR** = Security Orchestration, Automation & Response: the "brain" that receives an alert and *decides and executes* a response; **SOAR-lite** is AXIOM's ~200-line FastAPI stand-in (a design, not a deployed service) for one.
- **What it actually is** — Commercial SOAR platforms (Tines, Splunk SOAR, Cortex XSOAR) are visual playbook engines: an event comes in, a playbook branches ("is this host in the enclave? is the policy critical?"), calls out to other tools, and either auto-remediates or opens a ticket. AXIOM doesn't need — and at $0 can't justify — a full SOAR, so it **specifies** a **SOAR-lite** design: a single FastAPI service that *would* expose a couple of webhook routes and a small dispatch table mapping `policy_id → action`. It's the "orchestration + decision" node; Fleet supplies the "response" muscle (the script API). Analogy: SOAR-lite is a **911 dispatcher** — it doesn't put out the fire, it takes the call, decides *which* truck to send, and dispatches it (or, for a cat in a tree, files a non-urgent report instead).
...
- **Where it sits in the stack** — *In the intended design,* a container on the `axiom-core` [Docker network](./02-containers-and-docker.md), *beside* Fleet. Upstream (input): Fleet's [webhooks](#1-webhook--the-push-contract). Downstream (output): back to the [Fleet REST API](#3-fleet-rest-api--api-tokens) for scripts, and *sideways* to a ticket file / Grafana / the host's NIC depending on the playbook.
...
- **Gotchas / myth-busting** — (1) **A canary ≠ FIM.** FIM watches *real* files that legitimately change (noisy); a canary is a file that should *never* change — the high-signal subset. Don't conflate the tripwire with the general monitor. (2) **Detective, not preventive** — like all FIM, it tells you *after* the touch; it doesn't block the read, so containment races the attacker (pair it with a real prevention control in production). (3) **Make it believable** — a canary named `DO_NOT_TOUCH_HONEYPOT.txt` catches no one; it must look like something worth stealing. (4) **Disambiguation — both senses live in this repo.** "Canary" *here* is the security **honeytoken** (this file's enclave-FIM tripwire). But AXIOM *also* implements the **canary-deployment** sense — a real progressive-rollout cohort: the `/etc/axiom/canary` marker (`gitops/default.yml`) keys a `canary` cohort label that's promoted through a telemetry-gated [`gitops/promote/promote.ps1`](../adr/0009-canary-progressive-rollout.md), proven live on `canary-01`. So the two meanings **coexist** in this lab — same word, two mechanisms — see [ADR-0009 · canary progressive rollout](../adr/0009-canary-progressive-rollout.md); don't assume one excludes the other. (5) **A canary you forget to monitor is just a file** — the signal only exists if `file_events` is actually watching the path and a policy is actually evaluating it.
